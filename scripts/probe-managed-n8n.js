...
async function main() {
  ...
  console.log('Authorization:  ', authz ? `${authz.slice(0, 18)}…  (len=${authz.length})` : '(none)')
  ...
  console.log('X-N8N-API-KEY on destination:', apiKeyOnDest ? `${apiKeyOnDest.slice(0, 12)}…  (len=${apiKeyOnDest.length})` : '(none)')
  ...
  console.log('apiKey:         ', conn.apiKey ? `${conn.apiKey.slice(0, 12)}…  (len=${conn.apiKey.length})` : '(none)')
