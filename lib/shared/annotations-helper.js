 ...
 *   extractQualifier('@n8n.trigger#one', '@n8n.trigger') → 'one'
 *   extractQualifier('@n8n.trigger',      '@n8n.trigger') → undefined
 */
 ...
 * '.' after the base.
 *
 *   getAnnotationPrefixes(entity, '@n8n.trigger')
 *     → Set { '@n8n.trigger', '@n8n.trigger#one' }
 *
 * NOTE: Only prefixes that have at least one sub-key (`.workflow`, `.on`, …)
 * are returned; the pure string-shorthand form (`@n8n.trigger: 'foo'`) is
 * handled separately via `extractStringShorthand`.
 */
...
/**
 * Detects the string-shorthand form of `@n8n.trigger`. When users write:
 *   @n8n.trigger: 'my-webhook'
 * the CDS compiler stores the value on the base annotation key directly
 * (without a `.workflow` sub-key). Returns the string or undefined.
 *
 * Also handles qualified string shorthands like `@n8n.trigger#one: 'foo'`.
 */
...
/**
 * Collects `@n8n.trigger[.…]` annotation descriptors from an entity or event
 * definition. Handles both:
 *   - Record form: @n8n.trigger: { workflow, on, if, inputs }
 *   - String shorthand: @n8n.trigger: 'my-webhook'      (fires on CREATE + UPDATE)
 * Qualified variants (`@n8n.trigger#foo: …`) are handled the same way.
 *
...
function findTriggerAnnotations(entity) {
  ...
  // Record form (has sub-keys like `.workflow`, `.on`, `.if`, `.inputs`)
  const prefixes = getAnnotationPrefixes(entity, N8N_TRIGGER)
  for (const prefix of prefixes) {
    seenPrefixes.add(prefix)

    const workflowKey = `${prefix}.workflow`
    const onKey = `${prefix}.on`
    const ifKey = `${prefix}.if`
    const inputsKey = `${prefix}.inputs`

    const workflow = entity[workflowKey]
    if (!workflow || typeof workflow !== 'string') continue

    const onRaw = entity[onKey]
    const on = normalizeOnValue(onRaw)
    if (!on || on.length === 0) continue

    const qualifier = extractQualifier(prefix, N8N_TRIGGER)
  ...
  for (const key of Object.keys(entity)) {
    ...
    // Skip sub-keys of a record-form prefix (e.g. '@n8n.trigger.workflow')
    ...
    results.push({
      qualifier: extractQualifier(key, N8N_TRIGGER),
      workflow: shorthand,
