...
// Install @n8n.trigger annotation handlers on each ApplicationService as it is
// served. Mirrors process/lib/index.ts's serving hook.
cds.on('serving', (service) => {
  try {
    registerAnnotationHandlers(service)
  } catch (err) {
    LOG.error(`Failed to register @n8n.trigger handlers for ${service?.name}:`, err.message ?? err)
  }
})
