...
const { N8N_PROCESS_START, CUD_EVENTS } = require("../constants")
const { getAnnotationPrefixes, extractStringShorthand } = require("../shared/annotations-helper")

const ALLOWED_SUFFIXES = ["path", "on", "if", "inputs"]
...
function validateTriggerAnnotations(entityName, def, plugin) {
  ...
  // Discover string-shorthand keys ("@n8n.process.start" or
  // "@n8n.process.start#foo" with a plain string value). No structured
  // validation to do beyond "value is a non-empty string" (checked at scan
  // time), so we simply note them for completeness.
  ...
  const prefixes = getAnnotationPrefixes(def, N8N_PROCESS_START)
  for (const prefix of prefixes) {
    ...
    // Both path and on required for record form.
    if (hasPath !== hasOn) {
      report(plugin, ERROR, {
        entity: entityName,
        annotation: prefix,
        message: `${prefix}: both ${pathKey} and ${onKey} must be present together when using the record form.`,
      })
      continue
    }

    if (!hasPath) continue // nothing more to validate
