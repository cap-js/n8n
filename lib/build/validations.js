...
// Returns unique record-form prefixes (`@n8n.process.start`, `@n8n.process.start#foo`).
// Only prefixes that carry at least one sub-key (`.path`, `.on`, …) are included.
function getAnnotationPrefixes(def, base) {
  const prefixes = new Set()
  for (const key of Object.keys(def)) {
    if (!key.startsWith(base)) continue
    const dotIdx = key.indexOf(".", base.length)
    if (dotIdx === -1) continue
    prefixes.add(key.slice(0, dotIdx))
  }
  return prefixes
}
...
function validateTriggerAnnotations(entityName, def, plugin) {
  ...
  // "@n8n.process.start#foo" with a plain string value). Only structural
  // check: value must be a non-empty string.
  for (const key of Object.keys(def)) {
  ...
  for (const prefix of prefixes) {
    ...
    // Record form requires `path`. `on` is optional (defaults to all CRUD).
    if (!hasPath) {
      if (hasOn) {
        report(plugin, ERROR, {
          entity: entityName,
          annotation: prefix,
          message: `${prefix}: ${pathKey} is required (found only ${onKey}).`,
        })
      }
      continue
    }
