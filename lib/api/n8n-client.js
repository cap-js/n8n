...
/**
 * Validates and normalises a workflow value into a webhook path fragment.
 *   'my-hook'          → 'my-hook'
...
function normalizeWebhookPath(workflow) {
  ...
  if (/^[a-z][a-z0-9+.-]*:/i.test(trimmed) || trimmed.startsWith('//')) {
    throw new cds.error(400, `n8n: workflow must be a relative path, not a URL: ${safeForLog(trimmed)}`)
  }
  if (/[\r\n]/.test(trimmed)) {
    throw new cds.error(400, 'n8n: workflow must not contain newline characters')
  }
  const stripped = trimmed.replace(/^\/+/, '').replace(/^webhook\//, '')
  if (stripped.split('/').some((seg) => seg === '..')) {
    throw new cds.error(400, `n8n: workflow must not contain ".." segments: ${safeForLog(trimmed)}`)
  }
  return stripped
}
...
function createN8nClient(resolveConnection) {
  return {
    async trigger(workflow, payload) {
      ...
      return trigger(baseUrl, apiKey, workflow, payload, timeout, { useTestWebhook, authHeaders })
    },
