...
function resolveUseTestWebhook(cfg) {
 ...
 *
 * Precedence:
 *   1) BTP destination named in `credentials.destination`. Wins outright —
 *      otherwise a leftover `credentials.url` would silently shadow a
...
async function resolveN8nConnection() {
  ...
  const destinationName = creds.destination
  if (destinationName) {
  ...
  if (creds.url) {
    ...
    return {
      ...
      // !!cfg?.credentials?.useTestWebhook
      useTestWebhook: resolveUseTestWebhook(cfg),
      authHeaders: {},
      webhookAuthHeaders,
    }
