...
/**
 * Filters empty / whitespace-only strings to `undefined` so precedence chains
 * can fall through cleanly. Non-strings pass through unchanged.
 */
...
/**
 * Headers for n8n *webhook* calls (i.e. `/webhook/…` POSTs). n8n's Webhook
 * node does not mandate a header name for auth - workflow authors pick their
 * own via the "Header auth" credential - but we align on `X-N8N-API-KEY`
 * because that's the header n8n itself uses for its public REST API. Using
 * a single header name across the whole plugin surface keeps the setup
 * story simple: one API key, one header, everywhere.
 */
...
/**
 * Headers for n8n's public REST API (`/api/v1/…`). n8n enforces
 * `X-N8N-API-KEY` on these endpoints.
 */
 ...
 *   1) `credentials.useTestWebhook` (bool or string)
...
function resolveUseTestWebhook(cfg, destination) {
  ...
  return (
    toBool(nonEmpty(creds.useTestWebhook)) ??
 ...
 *   1) `credentials.timeout.{connect,read}`
...
function resolveTimeouts(cfg) {
  ...
  const connect =
    toPositiveInt(nonEmpty(t.connect)) ??
    toPositiveInt(process.env.N8N_CONNECT_TIMEOUT_MS) ??
    DEFAULT_CONNECT_TIMEOUT_MS
  const read =
    toPositiveInt(nonEmpty(t.read)) ??
 ...
 *   2) Bound / inline `credentials.{baseUrl,apiKey}`.
...
async function resolveN8nConnection(serviceName = N8N_SERVICE) {
  ...
  // 2) Bound / inline credentials. `apiKey` falls through to env var so
  //    a base URL configured in package.json + a key from `.env` compose
  //    naturally (very common for `cds bind` / `.env` workflows).
  const credBaseUrl = nonEmpty(creds.baseUrl ?? creds.url)
  const credApiKey = nonEmpty(creds.apiKey) ?? nonEmpty(process.env.N8N_API_KEY)
  if (credBaseUrl) {
 ...
 *
 * - `apiKey`: the n8n API key. Sent as `X-N8N-API-KEY` on both webhook POSTs
 *   and `/api/v1` calls - a single header name for the entire plugin surface.
