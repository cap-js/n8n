...
async function resolveN8nConnection(serviceName = N8N_SERVICE) {
  ...
  if (credBaseUrl) {
    ...
    return buildConnection(credBaseUrl, credApiKey, timeout)
  }
  ...
  if (destinationName) {
    ...
    return buildConnection(destination.url, apiKey, timeout)
  }
  ...
  if (envBaseUrl) {
    ...
    return buildConnection(envBaseUrl, envApiKey, timeout)
  }
  ...
  if (isDev) {
    ...
    return buildConnection(DEV_DEFAULT_BASE_URL, undefined, timeout)
  }
...
/**
 * Uniform connection shape passed to the client. `headers` is kept for
 * backwards compatibility (older callers may destructure it) and mirrors the
 * legacy `X-N8N-API-KEY`-only set; the client itself now picks the right
 * header set per endpoint via `apiKey`.
 */
function buildConnection(baseUrl, apiKey, timeout) {
  return {
    baseUrl,
    apiKey,
