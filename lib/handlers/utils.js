...
async function parseResponse(req, response) {
  ...
  LOG.error(`Error requesting ${req.target?.name ?? req.event} from n8n:`, response.status, detail)
  throw cds.error(502, `n8n request failed with status ${response.status}: ${detail}`)
}
