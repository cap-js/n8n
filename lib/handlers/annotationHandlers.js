...
const { CUD_EVENTS } = require("../constants")
...
/**
 * Registers @n8n.process.start annotation handlers on a given CAP service.
 * Only ApplicationServices carry entities/actions we care about; other
 * services (like the built-in outboxed n8n itself) are skipped.
 */
function registerAnnotationHandlers(service) {
  if (!(service instanceof cds.ApplicationService) || service.name === "N8nService") return

  const cache = buildTriggerCache(Object.values(service.entities ?? {}), CUD_EVENTS)
  if (cache.size === 0) return

  LOG.debug(
    `Registering @n8n.process.start handlers for service ${service.name} (${cache.size} entity:event bindings)`,
  )
  ...
  service.after("*", async (results, req) => {
    ...
    if (rows.length > 0) {
      await Promise.all(rows.map((row) => dispatchTriggers(cached.triggerAnnotations, req, row)))
    } else {
...
/**
 * Custom events annotated with @n8n.process.start fire when emitted on the
 * service. Register handlers for these events specifically.
 */
function registerEventTriggerHandlers(service) {
  ...
  const { findTriggerAnnotations } = require("../shared/annotations-helper")
