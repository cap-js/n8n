...
const DEFAULT_EVENTS = ["CREATE", "UPSERT", "UPDATE", "DELETE"]
...
function registerAnnotationHandlers(service) {
  ...
  // One before-DELETE per entity — batches column requirements across all
  // DELETE annotations on that entity into a single SELECT.
 ...
 * Both may be qualified: `@n8n.process.start#foo`.
 *
 * When `on` is not specified (either form), the annotation defaults to
 * firing on all CRUD events (see DEFAULT_EVENTS). An explicit empty array
 * `on: []` is a deliberate no-op — the annotation is kept but registers
 * no handlers.
 */
function* findAnnotations(def) {
  ...
  for (const [prefix, hasSubkeys] of byPrefix) {
    if (hasSubkeys) {
      ...
      const on = rawOn == null ? [...DEFAULT_EVENTS] : normalizeOn(rawOn)
