...
function* findAnnotations(def) {
  const hasRecordForm = ["path", "method", "on", "if", "inputs"].some((suffix) =>
...
function getRecordAnnotation(def) {
  ...
  return {
    path,
    method: normalizeMethod(def[`${N8N_PROCESS_START}.method`]),
    on,
...
function normalizeMethod(method) {
  return typeof method === "string" && method.trim() ? method.trim().toUpperCase() : "POST"
}
