...
const { safeForLog } = require('../api/n8n-client')
...
async function handleTrigger(req, data, annotation) {
  ...
  await emitTrigger(req, annotation.path, payload)
}
...
async function resolveEntityRow(req, data, conditionExpr, columns) {
  ...
  try {
    return await req.tx.run(SELECT.one.from(req.target.name).columns(columns).where(where))
  } catch (err) {
    LOG.error(`Failed to fetch row for @n8n.process.start on ${req.target.name}:`, err.message ?? err)
    return undefined
  }
}
...
/**
 * Builds a stable stash key for a given entity + key values. Uses JSON so
 * key values containing separator characters (`|`, `=`) do not collide.
 */
...
async function emitTrigger(req, path, payload) {
  // Delivery failures (HTTP errors from n8n) are handled inside the N8nService
  // itself: retryable failures throw so the outbox reschedules; unrecoverable
  // ones resolve with { ok:false } so the outbox marks the message done.
  //
  // Only *emit-time* failures reach this catch — most notably a broken
  // `cds.connect.to` (no such service) or a failing outbox persist. Both are
  // configuration bugs the caller wants to know about, so we propagate.
  ...
  try {
    ...
    // caller's transaction commits. When outbox is disabled (tests, console
    // kind), emit runs synchronously against the underlying service.
    await n8n.emit('trigger', { path, payload })
  } catch (err) {
    LOG.error(
      `Failed to enqueue n8n trigger for path ${safeForLog(path)}: ${safeForLog(err.message ?? err)}`,
    )
    throw err
  }
}
