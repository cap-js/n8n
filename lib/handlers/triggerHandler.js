 ...
 * 4. Emit `triggerWorkflow` on the outboxed n8n.
 */
async function handleTrigger(req, data, annotation) {
  ...
  await emitTrigger(annotation.path, row)
}

/**
 * Prefetch handler for DELETE. Runs once per entity in the before-DELETE
 * phase: fetches the union of columns required by every DELETE annotation
 * on this entity in a single SELECT and stashes the row on req.context so
 * the after-handler can read it back after the row is gone from the DB.
 */
async function prefetchForDelete(req, entity, annotations) {
  ...
  const columns = unionColumns(annotations.map((ann) => buildColumnsFromInputs(ann.inputs, entity)))
  ...
  try {
    ...
    LOG.error(`Failed to prefetch row for DELETE trigger on ${entity.name}:`, err.message ?? err)
    return
  }
  if (!row) return
...
/**
 * Merges two column lists into one, deduplicating by textual identity.
 * Shortcut to ['*'] when any list is the wildcard.
 */
function unionColumns(columnLists) {
  if (columnLists.length === 0) return [WILDCARD]
  if (columnLists.some((cols) => cols.length === 1 && cols[0] === WILDCARD)) return [WILDCARD]
...
async function emitTrigger(path, payload) {
  const n8n = await cds.connect.to("n8n")
  try {
    await n8n.emit("triggerWorkflow", { path, payload })
  } catch (err) {
    LOG.error(`Failed to enqueue n8n triggerWorkflow for path ${path}: ${err.message ?? err}`)
    throw err
  }
}
