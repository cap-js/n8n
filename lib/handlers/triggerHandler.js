...
async function handleTrigger(req, data, annotation) {
  ...
  await emitTrigger(annotation.path, row, annotation.method)
}
...
async function emitTrigger(path, payload, method) {
  const n8n = await cds.connect.to("n8n")
  try {
    await n8n.emit("triggerWorkflow", { path, method, payload })
  } catch (err) {
