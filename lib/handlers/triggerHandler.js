...
const { USER_ASSERTION_HEADER } = require("../auth/token-exchange")
...
async function sendTrigger(path, payload, method) {
  ...
  try {
    // Capture the authenticated user's JWT synchronously, while still in the
    // originating request context. Triggers are outboxed and delivered
    // asynchronously, where `cds.context.user.authInfo` is no longer available.
    // We forward it as a persisted message header so the (SAP-managed) gateway
    // adapter can perform the IAS assertion at delivery time. Other adapters
    // (console, rest) simply ignore it.
    const headers = buildAssertionHeaders()
    await n8n.send({ event: "trigger", data: { path, method, payload }, headers })
  } catch (err) {
...
/**
 * Extracts the raw user JWT from the current CAP context and returns it as a
 * header map. Prefers the token parsed by the @sap/xssec-based ias/jwt auth
 * strategies (`cds.context.user.authInfo.token.jwt`); falls back to the raw
 * `Authorization: Bearer <jwt>` header on the incoming HTTP request, which makes
 * local testing possible without a full IAS validation setup.
 *
 * Returns `undefined` when no token is present (e.g. mocked/basic auth), so no
 * header is attached and non-gateway adapters are unaffected.
 */
function buildAssertionHeaders() {
  const jwt = resolveUserJwt()
  if (!jwt) return undefined
  return { [USER_ASSERTION_HEADER]: jwt }
}

function resolveUserJwt() {
  const fromAuthInfo = cds.context?.user?.authInfo?.token?.jwt
