...
async function resolveEntityRow(req, data, annotation, columns) {
  ...
  // causing an unbounded recursion. Routing through cds.db hits the
  // persistence layer directly and preserves the current transaction.
  let runner
  if (req.event === "READ") {
    if (!cds.db) {
