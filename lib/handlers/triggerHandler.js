...
const { resolveUserJwt } = require("../api/connection")
...
async function sendTrigger(path, payload, method) {
  ...
  try {
    // Forward the user's JWT as a header so it survives the outbox and the
    // gateway adapter can assert it at delivery. Other adapters ignore it.
    // Only `x-correlation-id` propagates through the outbox context, so the
    // JWT must ride on the event headers rather than cds.context.
    const jwt = resolveUserJwt()
    const headers = jwt ? { [USER_ASSERTION_HEADER]: jwt } : undefined
