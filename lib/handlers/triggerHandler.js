...
function convertTreeToColumns(tree) {
  ...
  for (const node of tree) {
    ...
    if (node.targetVariable) col.as = node.targetVariable
 ...
 * DELETE events, we return the incoming data as-is because the row is already
 * gone at the point our `after` handler runs.
 *
...
async function resolveEntityRow(req, data, conditionExpr, columns) {
  if (req.event === 'DELETE') return data
 ...
 * exactly those columns (with aliases applied by CAP), so the row shape is
 * already correct.
 */
...
async function emitTrigger(req, workflow, payload) {
  try {
    ...
    // When the N8nService kind has `outbox: true`, cds.connect.to already
    // returns an outboxed proxy — .emit() persists the event in the outbox
    // and dispatches it after the caller's transaction commits.
