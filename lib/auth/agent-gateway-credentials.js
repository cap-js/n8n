...
/**
 * Non-throwing check whether the n8n service is configured for the Agent Gateway
 * business connector. Used by the REST adapter to branch the trigger between a
 * direct webhook call and the gateway-gated IAS assertion. Full validation is
 * deferred to `resolveAgentGatewayCredentials()` once the gateway path is taken.
 * @returns {boolean}
 */
function hasAgentGatewayCredentials() {
  return Boolean(cds.env.requires?.n8n?.credentials?.agentGateway)
}

module.exports = { resolveAgentGatewayCredentials, hasAgentGatewayCredentials }
