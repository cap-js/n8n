...
describe("buildWebhookUrl", () => {
  ...
  it("rejects absolute path values (SSRF)", () => {
    expect(() => buildWebhookUrl("http://ignored", "https://cloud.n8n.io/webhook/x")).toThrow(
      /relative path/i,
    )
  })
  it("uses /webhook-test prefix when useTestWebhook is true", () => {
    expect(buildWebhookUrl("http://localhost:5678", "book-created", { useTestWebhook: true })).toBe(
      "http://localhost:5678/webhook-test/book-created",
    )
  })
