...
describe("parseInputsArray", () => {
  it("handles simple entries", () => {
    expect(parseInputsArray([{ "=": "$self.ID" }, { "=": "$self.total" }])).toEqual([
      { path: ["ID"] },
      { path: ["total"] },
    ])
  })
...
describe("buildInputTree", () => {
  it("emits a scalar node", () => {
    const tree = buildInputTree([{ path: ["ID"] }], makeContext({ ID: { isAssocOrComp: false } }))
  ...
  it("emits a composition with specific nested fields (wildcard NOT injected when only nested)", () => {
    ...
    expect(tree).toEqual([
      {
        sourceElement: "items",
        associatedInputElements: [{ sourceElement: "title" }],
      },
    ])
  })
  ...
  it("injects wildcard when combining expand-all with specific nested fields", () => {
    ...
    expect(tree).toEqual([
      {
        sourceElement: "items",
        associatedInputElements: [{ sourceElement: WILDCARD }, { sourceElement: "title" }],
      },
    ])
  })
})
